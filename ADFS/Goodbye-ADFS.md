...
### デバイス登録サービス (DRS)  
ADFS デバイス登録サービスを代替する Azure AD 側の実装方式は複数存在しています。自組織で用いるデバイス種別 (PC, モバイル, 各々の OS 種別バージョンなど) を鑑み、フィットする [Azure AD 上のデバイス管理方式を選択](https://docs.microsoft.com/ja-jp/azure/active-directory/device-management-introduction)し、デバイス管理のインフラ側設定を実施します。
