...
## モダン認証方式の定義
このドキュメントにおけるモダン認証方式の定義は、Azure AD や主要な IDaaS がサポートしているオープンな認証プロトコルを利用したもので、アプリの配置場所に影響されることはありませせん。実質的には、以下２つの認証プロトコルを利用したものに絞られます（WS-Federation 対応のアプリケーションを作成することはほぼ無いため、対象外とします）。
